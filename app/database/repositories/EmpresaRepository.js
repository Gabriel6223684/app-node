...
export default class EmpresaRepository {
  ...
  static async search({ draw, start = 0, length = 10, search = '' }) {
    ...
    try {
      const term = `%${search}%`;
      const totalResult = await client.query('SELECT count(*)::int AS total FROM empresa');
      const recordsTotal = parseInt(totalResult.rows[0].total);

      const filteredResult = await client.query(
        'SELECT count(*)::int AS filtered FROM empresa WHERE nome ILIKE $1 OR cnpj ILIKE $1',
        [term]
      );
      const recordsFiltered = parseInt(filteredResult.rows[0].filtered);

      const dataResult = await client.query(
        'SELECT * FROM empresa WHERE nome ILIKE $1 OR cnpj ILIKE $1 ORDER BY nome LIMIT $2 OFFSET $3',
        [term, length, start]
      );
